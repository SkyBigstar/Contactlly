...
class Login extends Component {
	...
	render() {
		...
		return (
			<>
				<GoogleOAuthProvider clientId="939151310813-n2jvktsh17jge08lkh137jhkkdshh1rk.apps.googleusercontent.com">
					<GauthComp store={this.props.store}></GauthComp>
				...
				<div className="container mx-auto lg:px-4 py-4 min-h-screen flex flex-col md:items-center md:justify-center">
					<div className="text-center mb-6">
						<Logo />
						<div className="text-3xl md:text-5xl relative font-medium text-transparent bg-clip-text bg-gradient-to-r from-gray-700 to-gray-600 mb-4">AI<span className="font-normal "> Assistant</span>
			...
			<form>
				...
				<div className="mt-3 text-center ">
					...
					<div className="flex flex-col">
						<button onClick={onLogin} type="submit" className="hover:bg-gray-600 font-medium rounded-lg text-lg px-4 py-2 bg-gray-500 text-white mt-4 border border-gray-300 inline-block" >
							Log in
						</button>
			...
			if (user) {
				axios
					...
					.then(async (res) => {
						...
						try {
							let data = await store.api.post('/auth/signup', {
	...
	return (
		<>
			<div className='absolute right-3 top-0'>
				<button onClick={() => login()} className="hover:bg-gray-600 font-medium rounded-lg text-lg px-4 py-2 bg-gray-500 text-white mt-4 border border-gray-300 inline-block">
					Sign in with <i className="fa-brands fa-google"></i>oogle
				</button>
			</div>
		</>
	)
